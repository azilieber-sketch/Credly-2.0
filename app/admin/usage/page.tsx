  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 sm:px-6 md:px-8 md:py-8">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-zinc-900">Usage</h1>
        <p className="text-sm text-zinc-400 mt-0.5">{platformTotal.toLocaleString()} credits consumed platform-wide</p>
      </div>

      {/* ── Trend chart ── */}
      <div className="bg-white rounded-xl border border-zinc-200 p-5 sm:p-6 mb-5">
          ...
          {months.map((m) => {
            ...
            return (
              <div key={m.label} className="flex flex-col items-center gap-1.5 flex-1">
                <span className="text-[9px] sm:text-[10px] text-zinc-400 tabular-nums hidden sm:block">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-5">
        {[
          ...
          <div key={c.label} className="bg-white rounded-xl border border-zinc-200 p-4 sm:p-5">
            <p className="text-[10px] sm:text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">{c.label}</p>
            <p className={`text-xl sm:text-2xl font-black tabular-nums ${c.color}`}>{c.value.toLocaleString()}</p>
              ...
              return (
                <tr key={row.id} className={`hover:bg-zinc-50 transition-colors ${i < rows.length - 1 ? "border-b border-zinc-100" : ""}`}>
                  <td className="px-5 py-3.5">
                    ...
                    {st !== "active" && <p className="text-[11px] text-zinc-400 capitalize mt-0.5">{st}</p>}
                  </td>
              ...
              <div key={row.id} className="px-4 py-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-zinc-500 font-medium tabular-nums">{sharePct}%</span>
                  </div>
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {[
                    { label: "Ship.", value: row.shipping },
                    { label: "Ret.",  value: row.returns  },
                    { label: "Inq.",  value: row.inquiries},
                    { label: "Total", value: row.used     },
                  ].map((s) => (
                    <div key={s.label}>
                      <p className="text-[10px] text-zinc-400 uppercase tracking-wider mb-0.5">{s.label}</p>
                      <p className="text-sm font-medium text-zinc-700 tabular-nums">{s.value.toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </div>
