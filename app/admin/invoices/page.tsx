    ...
    <div className="max-w-4xl mx-auto px-8 py-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-0.5">
            ${totalRevenue.toFixed(2)} collected · {pendingRevenue > 0 && <span className="text-amber-600">${pendingRevenue.toFixed(2)} pending</span>}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: "Pending",         value: `$${pendingRevenue.toFixed(2)}`, sub: `${invoices.filter(i => i.status === "pending").length} awaiting payment`, color: pendingRevenue > 0 ? "text-amber-500" : undefined },
          ...
          <div key={s.label} className="bg-white rounded-xl border border-zinc-200 p-5">
            ...
            <p className={`text-2xl font-black tabular-nums ${(s as { color?: string }).color ?? "text-zinc-900"}`}>{s.value}</p>
